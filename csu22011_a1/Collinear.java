...
class Collinear {
		...
		for(int i=0; i<a1.length; i++) {
			for(int j=0; j<a2.length; j++) {
				for(int k=0; k<a3.length; k++) {
					// This formula checks if the points are collinear
					// It would be clearer if I used constants, but I
 					// simplified them out for efficiency purposes
					count += ((-a1[i] + (2*a2[j]) - a3[k])==0 ? 1 : 0);
				}
			}
		}
		return count;
	...
	static int countCollinearFast(int[] a1, int[] a2, int[] a3) {
		...
		for(int i=0; i<a1.length; i++) {
			for(int j=0; j<a2.length; j++) {
				count += (binarySearch(a3, a2[j]+(a2[j]-a1[i])) ? 1 : 0);
			}
		}
		return count;
	}
